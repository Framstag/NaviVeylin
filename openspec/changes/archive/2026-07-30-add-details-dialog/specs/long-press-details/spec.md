...
The map canvas SHALL detect a long-press gesture: user presses and holds for 500ms without dragging. On detection, the screen coordinates SHALL be converted to geographic coordinates (latitude, longitude) using the current map projection and viewport.
...
5. **Visibility at current zoom** — types with `optimizeLowZoom` flag score higher at low zoom (magnification ≤ 12); types without the flag score higher at high zoom (magnification ≥ 8). Internal types score 0 (excluded). This prevents showing low-zoom-optimized landuse areas when zoomed in on a building, and prevents showing high-zoom detail POIs when zoomed far out.
