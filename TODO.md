...
## 14. Pre-existing zoom unit-test failures (unrelated to fix-address-lookup-accuracy)

- **Pre-existing** ✗: `MapCanvasViewModelAutoZoomCommitTest` (2 tests: "auto zoom commits fractional magnifications stepping half a level per update" 15.5 vs 15.55; "manual zoom suspends auto zoom and a band change re-engages with fractional convergence" 13.5 vs 13.45) and `MapCanvasViewModelNavEndRestoreTest` ("persisted zoom is the routing-end zoom", 13.0 vs -1.0) fail in the full `:app:testMobileDebugUnitTest` run. Verified on a CLEAN tree (stash of `app/src` edits, then rerun): failures persist. Attributed to the in-flight speed/zoom work (fix-stationary-speed-display / browse-drive-modes uncommitted files: `auto/AutoZoomController.kt`, `MapCanvasViewModel.kt`, speed tests). Handle in those changes, not in fix-address-lookup-accuracy.
