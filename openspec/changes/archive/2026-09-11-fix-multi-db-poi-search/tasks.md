...
- [x] 5.1 Install the rebuilt app with the multi-map setup that reproduced the bug (e.g. stale NRW extract + new Dortmund DB). Search near the Dortmund center: active map's results appear first, list is distance-ascending, no duplicated entries (spec scenarios). Inspect `adb logcat -s NaviVeylin` for the search
  - Verified on-device: with NRW + Dortmund datasets loaded, a search near the Dortmund center returns the bbox-containing (Dortmund) map's results first, merged list is distance-ascending, and no coordinate-duplicate rows appear.
- [x] 5.2 Verify single-map behavior on device: search returns distance-ascending results, identical object set to before (plus deterministic order)
  - Verified on-device: single-map search returns the same object set as before the change, now distance-ascending with deterministic ordering.
- [x] 5.3 Close the TODO.md §10 "Multi-DB POI search" entry and note the behavior change (merge + bbox-first + dedup + distance sort) in a one-line update; append any failed approach to `ki_processing_failures.log` per apply guidance
  - Done: TODO.md §10 row closed (status ✅, legend extended; one-line note documents behavior change: merge + bbox-first + dedup + coordinates-rounded dedup → distance-ascending + limit, verified 5.1/5.2). Failed approaches already logged during implementation (`ki_processing_failures.log` 2026-09-06T21:40Z: eco imports skip POI indexes; git stash around a copied submodule file loses the patch; JavaScout Maven tests need JAVA_HOME=java-21) — nothing new to append.
