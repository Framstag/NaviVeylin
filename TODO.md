...
- **Found 2026-09-13 while scaffolding `license-compliance-baseline`** ℹ: `openspec instructions <artifact>` prints `Rules for 'proposal' must be an array of strings, ignoring this artifact's rules` (same for `specs` and `design`) — the entries in `openspec/config.yaml` are written as a YAML block sequence, but three of them contain a colon followed by a space (lines 32, 41, 50), so YAML parses them as mappings and the whole array fails the array-of-strings check. Consequence: those three rule sets have never been applied to any change in this repository (the `tasks`/`apply`/`archive` sets are unaffected). **Fixed in the working tree during this session by quoting those three entries** (not by this change — the edit was made outside it); re-running `openspec instructions proposal` now emits the rules instead of the warning, confirmed after the fix. Worth a `openspec doctor` check in CI so a silently dropped rules file fails loudly in future.
