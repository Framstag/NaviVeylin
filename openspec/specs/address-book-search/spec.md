...
The map screen menu SHALL contain an "Address book" entry that opens the address-book person search. The entry SHALL be shown only while `READ_CONTACTS` is granted.

#### Scenario: Entry opens person search

- **WHEN** the user opens the map screen menu
- **AND** `READ_CONTACTS` is granted
- **THEN** an "Address book" menu entry SHALL be visible
...
- **THEN** the "Address book" entry SHALL NOT be shown in the map screen menu
