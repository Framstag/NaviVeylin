...
- [x] 7.1 Fresh install (no basemap): download basemap from map manager → return to map → basemap visible WITHOUT restart; verify `adb logcat -s NaviVeylin` shows "Basemap loaded from ..." and tile re-render logs after the download completes
- [x] 7.2 Delete basemap while app runs → map re-renders without basemap overlay, no restart
- [x] 7.3 Update basemap while app runs → new version visible after re-render, no restart
- [x] 7.4 Region-with-no-map viewport shows basemap borders/country names/coastlines after live install; regional map renders on top where covered (spec basemap-loading overlay requirements)
- [x] 7.5 Android Auto emulator/head-unit: repeat 7.1–7.3 on the car surface (MapScreen/NavigationScreen), verify surface refreshes after download (surface lifecycle per guidelines/UI.md parity requirement)
